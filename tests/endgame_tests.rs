...
#[cfg(test)]
mod tests {
    use rust_fish::{chess_functionality::{game_over::{is_checkmate, is_stalemate}, moves::calculate_possible_moves}, Piece, PieceType};
    ...
    fn checkmate_scenario() {
        ...
        assert!(!is_stalemate(&mut board, true, &None).unwrap());
        assert!(is_checkmate(&mut board, true, &None).unwrap());
    }
